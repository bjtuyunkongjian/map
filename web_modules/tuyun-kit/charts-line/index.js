...
export default class ChartsLine extends Component {
  ...
  _renderLegend = () => {
    ...
    for (let item of this._legendArr) {
      ...
      item.circle = this._createLegendCircle(item); // 上面的圆
      item.triangle = this._createLegendTriangle(item); // 上面的小三角
    }
  ...
  _createLegendTriangle = unit => {
    const { legendW, legendH, legendCenter } = unit;
    const { x: cx, y: cy } = legendCenter;
    const _r = Math.min(legendW, legendH) / 2 / 2;
    const _path2D = new Path2D();
    const _theta = Math.PI / 3;
    const _cosTheta = Math.cos(_theta);
    const _sinTheta = Math.sin(_theta);
    _path2D.moveTo(cx + _r, cy);
    _path2D.lineTo(cx - _r * _cosTheta, cy + _r * _sinTheta);
    _path2D.lineTo(cx - _r * _cosTheta, cy - _r * _sinTheta);
    _path2D.lineTo(cx + _r, cy);
    return _path2D;
  };

  _createLegendCircle = unit => {
    const { legendW, legendH, legendCenter } = unit;
    const { x: cx, y: cy } = legendCenter;
    const _r = Math.min(legendW, legendH) / 1.5 / 2;
    const _path2D = new Path2D();
    _path2D.arc(cx, cy, _r, 0, Math.PI * 2);
    return _path2D;
  };
  ...
  _renderLinePoint = () => {
    ...
    const _chartH = this._chartBottom - this._curHeight;
    this._chartH = _chartH; // 图表高度
    const _maxData = Math.ceil(this._maxData * 1.1) || 1;
    const _equalSize = _chartH / _maxData; // 按 _maxData 均分，每一份的大小
    for (let item of this._pointsArr) {
      ...
      for (let i = 0; i < data.length; i++) {
        const _val = data[i];
        item.x.push(i * _pIntreval + pointMargin);
  ...
  _createCenterLine = unit => {
    ...
    _path2D.lineTo(lineX, height + y - pointMargin);
    return _path2D;
  };
  ...
  _onClick = () => {
    ...
    for (let index = 0; index < this._legendArr.length; index++) {
      ...
      if (this._ctx.isPointInPath(unit.backRect, _ratioX, _ratioY)) {
