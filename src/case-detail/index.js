...
import { FetchDetailNum, FetchCaseData } from './webapi';
...
export default class CaseDetail extends Component {
  ...
  render() {
    const { visible, hidden, detailArr, selectedCode } = this.state;
  ...
  _fetchCaseData = async () => {
    ...
    const _bounds = _MAP_.getBounds();
    const _zoom = _MAP_.getZoom();
    const { res, err } = await FetchCaseData({
      points: _bounds,
      caseFirType: pCode,
      caseSecType: selectedCode || null,
      beginTime: this._start,
      endTime: this._end,
      level: _zoom
    });
