...
export default class ProgressBar extends Component {
  state = {
    ...
    detailBoxLeft: 0,
    hoverDate: '',
    policeType: '' // 案件类型 0：刑事 1：行政 2：全部
  };
  ...
  _dealWithEvent = () => {
    const {
      changeSelectedSituationTendency,
      changeSituationDate
    } = GloEventName;
    GlobalEvent.on(changeSituationDate, this._changeSituationDate);
  ...
  _changeSituationDate = async param => {
    // 重新开始播放
    Event.emit(EventName.togglePlay, false);
    await this.setState({ curPlayIndex: 0 });
    RemoveLayer(_MAP_, PoliceLayerId);
    RemoveLayer(_MAP_, PoliceLayerLightId);
    const {
      endYear,
      endMonth,
      endDate,
  ...
  _changePoliceType = async policeType => {
    // 重新开始播放
    Event.emit(EventName.togglePlay, false);
    await this.setState({ policeType, curPlayIndex: 0 });
    RemoveLayer(_MAP_, PoliceLayerId);
    RemoveLayer(_MAP_, PoliceLayerLightId);
  ...
  _fetchProgressData = async () => {
    const { policeType } = this.state;
    if (!policeType) return;
    ...
    // 循环请求
    //minX=&code=&beginTime=&endTime=&level=&minY=&maxX=&maxY=&split=
    ...
    for (let item of _unitDateArr) {
      ...
      const _param = `minX=${_bounds._sw.lng}&maxX=${_bounds._ne.lng}&minY=${
        _bounds._sw.lat
      }&maxY=${_bounds._ne.lat}&beginTime=${item[0]}&endTime=${
        item[item.length - 1]
      }&split=${item.length}&code=${policeType}&level=${_zoom}`;
