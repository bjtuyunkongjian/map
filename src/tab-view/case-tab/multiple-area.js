...
export default class MultipleArea extends Component {
  ...
  _dealWithEvent = () => {
    ...
    Event.on(EventName.changeCaseDensity, this._onCancelSelect); // 更改选中的案件平均密度，取消选中
  };
  ...
  // minX=&maxX=&minY=&maxY=&startTime=&endTime=&pitch=&type=&level=
  _fetchAreaAggregation = async () => {
    ...
    const _param = `minX=${_bounds._sw.lng}&maxX=${_bounds._ne.lng}&minY=${
      ...
      this._end
    }&pitch=${_pitch}&type=${checkedOpt}&level=${_zoom}`;
...
const multipleOpts = [
  { label: '全部案件', value: 'QBAJ' },
  { label: '行政案件', value: 'XZAJ' },
  { label: '刑事案件', value: 'XSAJ' }
];
