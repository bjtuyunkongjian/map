export const BusinessStatus = {
  '1': '歇业',
  '2': '停业',
  '3': '转业',
  '4': '正常',
  '5': '注销'
}; // 经营状态

export const UnitCategory = {
  '240': '娱乐服务',
  '216': '旧货',
  '219': '汽车租赁',
  '217': '金银加工',
  '220': '印刷',
  '211': '旅馆',
  '212': '典当',
  '213': '公章',
  '215': '开锁',
  '214': '废旧金属收购',
  '218': '机动车拆装',
  '221': '机动车修理',
  '280': '上网场所',
  '291': '保安',
  '292': '管制工具',
  '230': '危爆行业',
  '251': '新闻',
  '259': '学校（教育单位）',
  '269': '交通枢纽',
  '268': '加油站',
  '252': '国防科研',
  '271': '党政机关',
  '253': '电信',
  '254': '物流',
  '255': '银行',
  '256': '能源',
  '257': '物资储备'
}; // 单位类别

export const BaseInfo = [
  { label: '单位名称', value: '', key: 'dzmc' },
  { label: '三防设施', value: '', key: 'sfss' },
  { label: '重点单位', value: '', key: 'zddwbs' },
  { label: '单位地址', value: '', key: 'dwdz_qhnxxdz' },
  { label: '九小场所', value: '', key: 'sfjxcs' },
  { label: '安全通道数', value: '', key: 'aqcktds' },
  { label: '注册资金', value: '', key: 'zczj' },
  { label: '联系电话', value: '', key: 'lxdh' },
  { label: '网址', value: '', key: 'wz' },
  // { label: '单位类型', value: '', key: 'dwlx' },
  { label: '单位分类', value: '', key: 'dwfl' },
  { label: '从业人数', value: '', key: 'cyryzs' },
  { label: '主管单位', value: '', key: 'zgdw' },
  { label: '备案时间', value: '', key: 'gajgbasj' }
]; // 基本信息

export const DefendingPerInfo = [
  { label: '姓名', value: '', key: 'bwfzr_xm' },
  { label: '联系电话', value: '', key: 'bwfzr_lxdh' }
]; // 保卫负责人

export const BusinessInfo = [
  { label: '主营', value: '', key: 'zy' },
  { label: '兼营', value: '', key: 'jy' },
  { label: '营业方式', value: '', key: 'jyfs' },
  { label: '经营状态', value: '', key: 'jyzt' },
  { label: '经营面积', value: '', key: 'jymj' },
  { label: '营业执照', value: '', key: 'sfyyyzz' },
  { label: '营业执照号', value: '', key: 'yyzzh' }
]; // 经营范围

export const LegalRepInfo = [
  { label: '联系电话', value: '', key: 'frdb_lxdh' },
  { label: '身份证号码', value: '', key: 'frdb_sfzhm' },
  { label: '姓名', value: '', key: 'frdb_xm' }
]; // 法定代表人
